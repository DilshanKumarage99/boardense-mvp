...
def get_exit_readiness(company_id):
    ...
    try:
        report = generate_exit_readiness_report(company)
        return jsonify(report), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@reports_bp.route('/companies/<company_id>/investor-questions', methods=['GET'])
@jwt_required()
def get_investor_questions(company_id):
    user_id = get_jwt_identity()
    ...
    try:
        report = generate_investor_questions_report(company)
        return jsonify(report), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
